...
function JobsSection({ setActiveContent, setJobPostId }) {
  ...
  const [searchData, setSearchData]= useState({
      ...
      useEffect(()=>{
        ...
        if(searchData.jobTitle==""  && searchData.recruiter=="" && searchData.status=="" ){
        ...
        const timer=setTimeout(()=>{
          const data=jobsData.filter((data)=>{
            const jobTitleMatch=data?.role.toLowerCase().startsWith(searchData.jobTitle.toLowerCase().trim());
            const recruiterMatch=data.recruiter?.userName.toLowerCase().startsWith(searchData.recruiter.toLowerCase().trim());
            const statusMatch=(searchData.status==checkJobStatus(data.closingDate) ||  searchData.status=="");
...

            {
              ...
              <td >
                <div className="statusBox" >
                  <span className={`statusBadge text-uppercase  ${checkJobStatus(data.closingDate)}`}>{checkJobStatus(data.closingDate)}</span>
                </div>
              </td>
              <td>
                <button className="viewBtn d-flex align-items-center gap-1" onClick={() => {setJobPostId(data._id);setActiveContent("jobDetail")}} >
                  <FaEye />
                </button>
              </td>
            </tr>
