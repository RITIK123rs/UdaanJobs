...
router.put("/updateJobSeekerStatus",async (req,res)=>{
    ...
    const updateJobSeekerData=await jobSeeker.findOneAndUpdate({_id: jobSeekerObjId ,"application.appliedJobs.jobId": jobIdObjId },{$set: {"application.appliedJobs.$.status": jobStatus }, $inc: { [`application.${jobStatus}`]: 1, [`application.${oldJobStatus}`]: -1 } },{new: true});
...
router.delete("/deletePost/:id",async (req,res)=>{
    ...
    const DeleteJobPost = await jobPosted.findById(jobPostId).select("recruiter applications");
